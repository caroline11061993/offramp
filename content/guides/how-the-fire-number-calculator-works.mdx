...
4. From retirement onward, each year's spending (plus any gradual debt repayment) is drawn from liquid assets first, and only from pension once you've reached the UK pension access age (57) — exactly the same locking rule as the FIRE Age Calculator. Liquid withdrawals are pound-for-pound; pension withdrawals beyond the 25% tax-free element are taxed as UK income, so the calculator grosses up the withdrawal to cover what's actually needed rather than draining the pot pound-for-pound. This engine doesn't model a State Pension or DB pension at all, so there's nothing else for that tax to stack against.
